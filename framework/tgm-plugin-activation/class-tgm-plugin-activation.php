...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
    ...
    class TGM_Plugin_Activation {
        ...
        public function init() {
            ...
            // Load class strings.
            $this->strings = array(
                'page_title'                      => esc_html__( 'Install Required Plugins', 'hyperx' ),
                'menu_title'                      => esc_html__( 'Install Plugins', 'hyperx' ),
                'installing'                      => esc_html__( 'Installing Plugin: %s', 'hyperx' ),
                'oops'                            => esc_html__( 'Something went wrong.', 'hyperx' ),
                'notice_can_install_required'     => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'hyperx' ),
                'notice_can_install_recommended'  => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'hyperx' ),
                'notice_cannot_install'           => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'hyperx' ),
                'notice_can_activate_required'    => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'hyperx' ),
                'notice_can_activate_recommended' => _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'hyperx' ),
                'notice_cannot_activate'          => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'hyperx' ),
                'notice_ask_to_update'            => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'hyperx' ),
                'notice_cannot_update'            => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'hyperx' ),
                'install_link'                    => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'hyperx' ),
                'activate_link'                   => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'hyperx' ),
                'return'                          => esc_html__( 'Return to Required Plugins Installer', 'hyperx' ),
                'dashboard'                       => esc_html__( 'Return to the dashboard', 'hyperx' ),
                'plugin_activated'                => esc_html__( 'Plugin activated successfully.', 'hyperx' ),
                'activated_successfully'          => esc_html__( 'The following plugin was activated successfully:', 'hyperx' ),
                'complete'                        => esc_html__( 'All plugins installed and activated successfully. %1$s', 'hyperx' ),
                'dismiss'                         => esc_html__( 'Dismiss this notice', 'hyperx' ),
            );
         ...
         * here we have to require a file that has a function that does the
         ...
         * @since 2.1.0
         *
         * @global string $royal_tab Used as iframe div class names, helps with styling
         * @global string $royal_body_id Used as the iframe body ID, helps with styling
        ...
        public function admin_init() {
            ...
            if ( isset( $_REQUEST['tab'] ) && 'plugin-information' === $_REQUEST['tab'] ) {
                ...
                global $royal_tab, $royal_body_id;
                $royal_body_id = $tab = 'plugin-information';
         ...
         *
         * Thickbox is not automatically has on all admin pages, so we must
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' === $_GET['tgmpa-install'] ) ) {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>', sprintf( esc_html( $this->strings['complete'] ), '<a href="' . esc_url( self_admin_url() ) . '">' . esc_html__( 'Return to the Dashboard', 'hyperx' ) . '</a>' ), '</p>';
         ...
         * their theme. If appropriate, it has a link to the form page where
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[ $type ], $count, 'hyperx' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( $this->parent_slug ) ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'hyperx' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( $this->parent_slug ) ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'hyperx' ) . '</a>'  : '';
    ...
    class TGMPA_List_Table extends WP_List_Table {
            ...
            foreach ( $this->tgmpa->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        ...
                        $table_data[ $i ]['source'] = esc_html__( 'Private Repository', 'hyperx' );
                    }
                    else {
                        // The plugin is pre-packaged with the theme.
                        $table_data[ $i ]['source'] = esc_html__( 'Pre-Packaged', 'hyperx' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[ $i ]['source'] = esc_html__( 'WordPress Repository', 'hyperx' );
                }

                $table_data[ $i ]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? esc_html__( 'Required', 'hyperx' ) : esc_html__( 'Recommended', 'hyperx' );

                if ( ! isset( $installed_plugins[ $plugin['file_path'] ] ) ) {
                    $table_data[ $i ]['status'] = sprintf( '%1$s', esc_html__( 'Not Installed', 'hyperx' ) );
                }
                elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[ $i ]['status'] = sprintf( '%1$s', esc_html__( 'Installed But Not Activated', 'hyperx' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[ $item['file_path'] ] ) ) {
                ...
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s">' . esc_attr_x( 'Install %2$s', '%2$s = plugin name in screen reader markup', 'hyperx' ) . '</a>',
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                ...
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s">' . esc_attr_x( 'Activate %2$s', '%2$s = plugin name in screen reader markup', 'hyperx' ) . '</a>',
        ...
        public function no_items() {
            ...
            printf( wp_kses_post( esc_html__( 'No plugins to install or activate. <a href="%1$s">Return to the Dashboard</a>', 'hyperx' ) ), esc_url( self_admin_url() ) );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => esc_html__( 'Plugin', 'hyperx' ),
                'source' => esc_html__( 'Source', 'hyperx' ),
                'type'   => esc_html__( 'Type', 'hyperx' ),
                'status' => esc_html__( 'Status', 'hyperx' ),
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'tgmpa-bulk-install'  => esc_html__( 'Install', 'hyperx' ),
                'tgmpa-bulk-activate' => esc_html__( 'Activate', 'hyperx' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'tgmpa-bulk-install' === $this->current_action() ) {
                ...
                if ( empty( $plugin_installs ) ) {
                    echo '<div id="message" class="error"><p>', esc_html__( 'No plugins are available to be installed at this time.', 'hyperx' ), '</p></div>';
                    return false;
                }
            ...
            if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
                ...
                if ( empty( $plugins_to_activate ) ) {
                    echo '<div id="message" class="error"><p>', esc_html__( 'No plugins are available to be activated at this time.', 'hyperx' ), '</p></div>';
                    return false;
                }
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s.</p></div>', esc_html( _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'hyperx' ) ), wp_kses_post( $imploded ) );
                }
 ...
 * doesn't try to re-get the class when processing upgrades or installs outside
 * of the class.
 *
 * @since 2.2.0
 */
            ...
            if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
                ...
                class TGM_Bulk_Installer extends WP_Upgrader {
                    ...
                    public function install_strings() {
                        ...
                        $this->strings['no_package']          = esc_html__( 'Install package not available.', 'hyperx' );
                        $this->strings['downloading_package'] = esc_html__( 'Downloading install package from <span class="code">%s</span>&#8230;', 'hyperx' );
                        $this->strings['unpack_package']      = esc_html__( 'Unpacking the package&#8230;', 'hyperx' );
                        $this->strings['installing_package']  = esc_html__( 'Installing the plugin&#8230;', 'hyperx' );
                        $this->strings['process_failed']      = esc_html__( 'Plugin install failed.', 'hyperx' );
                        $this->strings['process_success']     = esc_html__( 'Plugin installed successfully.', 'hyperx' );
                    ...
                    public function activate_strings() {
                        ...
                        $this->strings['activation_failed']  = esc_html__( 'Plugin activation failed.', 'hyperx' );
                        $this->strings['activation_success'] = esc_html__( 'Plugin activated successfully.', 'hyperx' );
            ...
            if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
                ...
                class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                    ...
                    public function add_strings() {
                        ...
                        $this->upgrader->strings['skin_update_failed_error']  = esc_html__( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'hyperx' );
                        $this->upgrader->strings['skin_update_failed']        = esc_html__( 'The installation of %1$s failed.', 'hyperx' );

                        // Automatic activation strings.
                        if ( $this->tgmpa->is_automatic ) {
                            $this->upgrader->strings['skin_upgrade_start']        = esc_html__( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'hyperx' );
                            $this->upgrader->strings['skin_update_successful']    = esc_html__( '%1$s installed and activated successfully.', 'hyperx' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . esc_html__( 'Show Details', 'hyperx' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'hyperx' ) . '</span>.</a>';
                            $this->upgrader->strings['skin_upgrade_end']          = esc_html__( 'All installations and activations have been completed.', 'hyperx' );
                            $this->upgrader->strings['skin_before_update_header'] = esc_html__( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'hyperx' );
                        }
                        // Default installation strings.
                        else {
                            $this->upgrader->strings['skin_upgrade_start']        = esc_html__( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'hyperx' );
                            $this->upgrader->strings['skin_update_successful']    = esc_html__( '%1$s installed successfully.', 'hyperx' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . esc_html__( 'Show Details', 'hyperx' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'hyperx' ) . '</span>.</a>';
                            $this->upgrader->strings['skin_upgrade_end']          = esc_html__( 'All installations have been completed.', 'hyperx' );
                            $this->upgrader->strings['skin_before_update_header'] = esc_html__( 'Installing Plugin %1$s (%2$d/%3$d)', 'hyperx' );
                        }
                    ...
                    public function bulk_footer() {
                        ...
                        if ( empty( $complete ) ) {
                            echo '<p>', sprintf( esc_html( $this->tgmpa->strings['complete'] ), '<a href="' . esc_url( self_admin_url() ) . '">' . esc_html__( 'Return to the Dashboard', 'hyperx' ) . '</a>' ), '</p>';
                            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                        }
